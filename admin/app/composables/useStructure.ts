...
export const useStructure = () => {
    ...
    /** 批量儲存整棵樹的排序：依層級送出每組兄弟節點，再刷新畫面與側邊欄 */
    const batchSaveSortOrder = async (rootList: any[]) => {
        const lists: any[][] = [];
        ...
        for (const list of lists) {
            const ok = await updateSortOrder(list, { silent: true });
            if (!ok) return;
        }
        await fetchData();
        await fetchDataForAside();
    ...
    // 扁平化樹狀結構以檢查 URL 重複
    const flattenStructureForUrlCheck = (items: any[]): any[] => {
        const result: any[] = [];
    ...
    const validateForm = (excludeId?: number | string): boolean => {
        submitError.value = "";
        // 先清除所有錯誤
        Object.keys(errors).forEach((key) => {
            // @ts-ignore
        ...
        // 檢查 URL 是否重複（URL 不為空時才檢查）
        if (form.url && form.url.trim() !== "") {
            const flattened = flattenStructureForUrlCheck(data.value || []);
            ...
            if (duplicate) {
                errors.url = "此 URL 已被其他層級使用，請使用其他 URL";
                isValid = false;
            }
        }
    ...
    const updateLevel = async (
        ...
        if (!validateForm(options.levelId)) return false;
