...
export const useFormPreview = (options?: UseFormPreviewOptions) => {
    ...
    const updatePreview = (
        ...
        // 保存圖片來源供 getCoverUrl/getSlideUrls 使用
        imageSources.value = sources
            ? {
                  cover: sources.cover ? { ...sources.cover } : undefined,
                  slide: sources.slide ? { ...sources.slide } : undefined
              }
            : {};
        ...
        if (sources) {
            ...
            if (sources.cover) {
                const coverUrl =
                    sources.cover.preview ||
                    sources.cover.formValue ||
                    data.cover ||
                    "";
            ...
            if (sources.slide) {
                const slideUrls =
                    sources.slide.previews ||
                    sources.slide.formValue ||
                    data.slide ||
                    [];
