...
interface Props {
    /**
     * 日期欄位的值（格式：YYYY-MM-DD），允許為空或 null
     */
...
// 將 CalendarDate 格式化為 yyyy-mm-dd 顯示
const formatDateToYYYYMMDD = (d: CalendarDate): string =>
...
// 日期選擇器值（允許為空，不使用預設日期）
const dateValue = shallowRef<CalendarDate | null>(
    parseDateString(props.modelValue)
);

// 日期選擇器文字（顯示為 yyyy-mm-dd）
const dateText = computed(() => {
    if (!dateValue.value) return props.placeholder;
    try {
        return formatDateToYYYYMMDD(dateValue.value);
    } catch {
