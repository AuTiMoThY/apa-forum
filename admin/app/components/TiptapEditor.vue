...
/** 對齊對應的 flex justify-content（text-align 對 block 圖片無效，改用 flex） */
const alignToFlexStyle: Record<string, string> = {
...
/** 延伸 Image：用 div + flex 包住圖片，讓置中/靠右等對齊能正確顯示 */
const ImageWithAlign = TiptapImage.extend({
    renderHTML({ node, HTMLAttributes }) {
        const align = node.attrs.textAlign || "left";
        const { style, ...rest } = HTMLAttributes;
        const wrapperStyle = alignToFlexStyle[align] || alignToFlexStyle.left;
        return [
            "div",
            { style: wrapperStyle, class: "tiptap-image-align-wrapper" },
        ...
        return [
            {
                ...
                getAttrs: (dom) => {
                    ...
                    if (
                        ...
                        const textAlignM = style.match(/text-align:\s*(\w+)/);
                        const justifyM = style.match(
                            /justify-content:\s*(flex-\w+|center)/
                        );
                        if (justifyM) {
                            ...
                            const jc = justifyM[1];
                            attrs.textAlign = jc && map[jc] ? map[jc] : null;
                        } else if (textAlignM) {
                            attrs.textAlign = textAlignM[1] ?? null;
                        }
                    }
                    return attrs;
                }
...
const props = withDefaults(
    defineProps<{
        modelValue: string;
        /** 圖片上傳端點，例如 /upload/pages；不傳則使用預設 /upload/image */
...
const { uploadImage, uploadToEndpoint } = useImageUpload();
...
const handleImageUpload = async (event: Event) => {
    ...
    try {
        const imageUrl = props.uploadEndpoint
            ? (await uploadToEndpoint(file, props.uploadEndpoint))?.url ?? null
...
/** 供 UInput 使用的獨立 ref，避免輸入不完整 hex 時觸發 UColorPicker 驗證錯誤 */
const textColorInput = ref<string>("#000000");
const backgroundColor = ref<string>("#ffffff");

/** 驗證是否為有效的 hex 顏色（#rgb 或 #rrggbb） */
...
watch(textColor, (newColor) => {
    // 同步到輸入框（UColorPicker 或 updateColorFromEditor 變更時）
...
// 監聽輸入框：僅在輸入有效 hex 時更新 textColor，避免 UColorPicker 拋錯
watch(textColorInput, (val) => {
    if (isValidHex(val)) {
        textColor.value = val;
    }
});
...
<template>
    <div class="tiptap-editor">
        ...
        <div
            v-if="editor"
            class="toolbar border-b border-gray-200 p-2 flex flex-wrap gap-1 sticky -top-6 bg-white z-10 rounded-lg">
            <!-- 文字顏色選擇器 -->
            <UPopover>
                <UTooltip text="文字顏色">
                    <UButton
        ...
        <!-- 編輯器內容 -->
        <div class="editor-content prose-content overflow-y-auto">
...
/* 圖片對齊 wrapper：用 flex 讓 block 圖片能置中/靠右 */
.editor-content :deep(.tiptap-image-align-wrapper) {
    width: 100%;
}

.editor-content :deep(.tiptap-image-align-wrapper img) {
    display: block;
    max-width: 100%;
    height: auto;
}
