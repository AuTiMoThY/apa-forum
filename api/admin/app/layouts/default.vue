...
const groups = computed((): CommandPaletteGroup<CommandPaletteItem>[] => {
    const base: CommandPaletteGroup<CommandPaletteItem>[] = [
        {
            id: "links",
            label: "Go to",
            items: links.value[0] as CommandPaletteItem[]
        }
    ];
    const products = productGroupItems.value;
    if (products.length > 0) {
        base.push({
            id: "products",
            label: "產品（開啟編輯）",
            ignoreFilter: true,
            items: products as CommandPaletteItem[]
        });
    }
    return base;
});
...
<template>
    <UDashboardGroup unit="rem">
        <UDashboardSidebar
            ...
            <template #header="{ collapsed }">
                <NuxtLink to="/">
                    ...
                    <span v-else class="text-2xl font-bold">M</span>
                </NuxtLink>
            </template>
        ...
        <UDashboardSearch
            ...
            :groups="groups"
            placeholder="搜尋選單或產品（至少 2 字，或輸入數字編號）…" />
        <slot />
    </UDashboardGroup>
</template>
