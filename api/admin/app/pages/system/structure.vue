...
const { data, loading, fetchData, updateSortOrder, batchSaveSortOrder, deleteLevel } =
    useStructure();
const rootBodyRef = ref<HTMLElement | null>(null);
let sortableStop: (() => void) | null = null;
/** 是否有未寫入資料庫的排序變更，用來顯示「更新排序」按鈕 */
...
/** 包一層以符合 TreeTableRow 的 onUpdateSortOrder 型別 (list) => Promise<void> */
const handleUpdateSortOrder = async (list: any[]) => {
...
const onSortCompleted = () => {
    sortDirty.value = true;
    setupRootSortable();
};

const handleSaveSortOrder = async () => {
    await batchSaveSortOrder(data.value || []);
                ...
                <div class="block md:hidden space-y-3">
                    <template
                        ...
                        <StructureTreeTableRow
                            ...
                            :on-update-sort-order="handleUpdateSortOrder"
                            :on-delete="handleDelete"
                            @refresh="fetchData"
                            @sort-completed="onSortCompleted" />
                    </template>
                    <div
