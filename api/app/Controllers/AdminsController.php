...
class AdminsController extends BaseController
{
    ...
    public function addAdmin()
    {
        ...
        try {
            $insertId = $this->userModel->insert([
                ...
                'note_finance' => $data['note_finance'] ?? null,
                'note_deliver' => $data['note_deliver'] ?? null,
                'note_purchase' => $data['note_purchase'] ?? null,
                'is_first_login' => 0, // 新建立帳號一律為已雜湊密碼
            ]);
    ...
    {
        // 從 AuthFilter 設定的 Header 取得登入者資訊；若缺失則回退 Session
        $session = session();
        $userDataJson = $this->request->getHeaderLine('X-User-Data');
        $userData = $userDataJson ? json_decode($userDataJson, true) : null;
        ...
        // 判斷是否為超級管理員（統一使用 BaseController 方法，並確保角色啟用）
        $isCurrentUserSuperAdmin = $currentUserId ? $this->isSuperAdmin($currentUserId) : false;
