...
class StructureController extends BaseController
{
    ...
    public function add()
    {
        ...
        $rules = [
            ...
            'alias' => 'permit_empty|min_length[1]|max_length[255]',
        ...
        try {
            ...
            if (!empty($insertData['url'])) {
                $existing = $this->SysStructureModel->where('url', $insertData['url'])->first();
                if ($existing) {
                    return $this->response->setStatusCode(ResponseInterface::HTTP_UNPROCESSABLE_ENTITY)->setJSON([
                        'success' => false,
                        'message' => '此 URL 已被使用',
                        'errors' => [
                            'url' => '此 URL 已被其他層級使用，請使用其他 URL'
                        ],
                    ]);
                }
            }
    ...
    public function update()
    {
        ...
        try {
            ...
            // 檢查 URL 是否重複（URL 有變更且不為空時才檢查，排除當前記錄）
            if (array_key_exists('url', $updateData) && !empty($updateData['url'])) {
                $existing = $this->SysStructureModel->where('url', $updateData['url'])->where('id !=', $id)->first();
                if ($existing) {
                    return $this->response->setStatusCode(ResponseInterface::HTTP_UNPROCESSABLE_ENTITY)->setJSON([
                        'success' => false,
                        'message' => '此 URL 已被使用',
